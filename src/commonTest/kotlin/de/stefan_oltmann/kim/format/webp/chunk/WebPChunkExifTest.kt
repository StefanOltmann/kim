...
/**
 * Tests reading the EXIF chunk of WebP files.
 *
 * The chunk carries raw TIFF bytes per the WebP specification, but some
 * writers add the JPEG style "Exif\0\0" header in front of it. ExifTool
 * warns "Improper EXIF header" for those and still reads them.
 */
class WebPChunkExifTest {
    ...
    fun testReadsExifWithImproperHeader() {
        ...
        val chunk = WebPChunkExif(
            "Exif\u0000\u0000".encodeToByteArray() + tiffBytes
        )

        val chunkWithoutHeader = WebPChunkExif(tiffBytes)
